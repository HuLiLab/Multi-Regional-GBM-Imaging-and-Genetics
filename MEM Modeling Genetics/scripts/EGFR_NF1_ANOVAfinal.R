...
rawdata$NF1_cnv[rawdata$NF1_cnv==0 ]="WT"
rawdata$NF1_cnv[rawdata$NF1_cnv==-1 ]="Loss-1"
rawdata$NF1_cnv[rawdata$NF1_cnv==-2 ]="Loss-2"
rawdata$NF1_cnv=factor(rawdata$NF1_cnv,levels=c("WT","Loss-1","Loss-2"))

#keep samples with genomics and imaging data
rawdata=rawdata[rowSums(rawdata[,5:9], na.rm = TRUE)>0,]
rawdata=rawdata[!is.na(rawdata$EGFR_cnv),]
rawdata=rawdata[!is.na(rawdata$NF1_cnv),]
...
for(j in 1:length(combos[,1])){
  data=rawdata[,c("MRI.contrast.enhancing.annotation",as.character(combos[j,2]),"EGFR_cnv","NF1_cnv","Patient_ID")]
  if(combos[j,1] != "Total"){
    data=data[data[,1]==as.character(combos[j,1]),] 
  }
  data=data[,-1]
  colnames(data)=c("Measurement","EGFR","NF1","Patient")
  ...
  model<-lmer(Measurement~EGFR*NF1+(1|Patient),data=data)
  
  qc=plot(model,ylab = "Residual",xlab = "Variable",main = paste("Transform:",combos[j,3]))
  png(file = paste(combos[j,1],combos[j,2],"EGFRvNF1_QC.png",sep="_"))
  print(qc) 
  dev.off()
  
  aov.res<-anova(model,type=2)
  aov.res$prop.var.explained<-100*aov.res[,1]/((var(data$Measurement,na.rm = TRUE)*((nrow(data)-1))))
  write.csv(aov.res,paste(combos[j,1],combos[j,2],"anova_EGFRvNF1.csv",sep="_"))
  
  em<-summary(emmeans(model,pairwise~EGFR:NF1,adjust="tukey"))
  if(dim(aggmeans)[1]==0){
  ...
  em.nf1=summary(contrast(emmeans(model,"NF1"),"tukey"))
  ...
  if(dim(aggnf1)[1]==0){
    em.nf1[,2]=paste(combos[j,1],combos[j,2],sep="_")
    aggnf1=em.nf1[,c(1,2,6)]
  }
  else {
    em.nf1[,2]=paste(combos[j,1],combos[j,2],sep="_")
    aggnf1=rbind.data.frame(aggnf1,em.nf1[,c(1,2,6)])
  }
  write.csv(aggnf1,paste(combos[j,1],combos[j,2],"nf1_contrasts.csv",sep="_"))
  ...
  if (length(sig_genotypes)>0){
    ...
    for(i in 1:length(sig_genotypes[,1])){
      ...
      model<-lmer(Measurement~Group+EGFR+NF1+(1|Patient),data=curr)
...
colnames(aggpval)=c("MRI","Measurment","EGFR","NF1","EGFR-NF1")
aggres[,2]=gsub(".Raw_Mean","",aggres[,2])
aggres=aggres[order(aggres[,1]),]

write.csv(aggres,"EGFR_NF1_percentvariance_attributed.csv")
write.csv(aggpval,"EGFR_NF1_pvalue.csv")
