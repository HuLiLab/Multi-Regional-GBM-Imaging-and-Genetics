...
for(j in 1:length(combos[,1])){
  data=rawdata[,c("MRI.contrast.enhancing.annotation",as.character(combos[j,2]),"IDH.mutation.status","CDKN2A_cnv","Patient_ID")]
  if(combos[j,1] != "Total"){
    data=data[data[,1]==as.character(combos[j,1]),] 
  }
  data=data[,-1]
  colnames(data)=c("Measurement","IDH","CDKN2A","Patient")
  ...
  model<-lmer(Measurement~IDH*CDKN2A+(1|Patient),data=data)
  ...
  em<-summary(emmeans(model,pairwise~IDH:CDKN2A,adjust="tukey"))
  if(dim(aggmeans)[1]==0){
  ...
  if (length(sig_genotypes)>0){
    ...
    for(i in 1:length(sig_genotypes[,1])){
      ...
      model<-lmer(Measurement~Group+IDH+CDKN2A+(1|Patient),data=curr)
...
colnames(aggpval)=c("MRI","Measurment","IDH","CDKNN2A","IDH-CDKN2A")
aggres[,2]=gsub(".Raw_Mean","",aggres[,2])
aggres=aggres[order(aggres[,1]),]


write.csv(aggres,"IDH_CDKN2A_percentvariance_attributed.csv")
write.csv(aggpval,"IDH_CDKN2A_pvalue.csv")
