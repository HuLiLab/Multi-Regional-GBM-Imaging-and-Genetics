...
#Annotate variants with appropriate labels 
...
for(j in 1:length(combos[,1])){
  data=rawdata[,c("MRI.contrast.enhancing.annotation",as.character(combos[j,2]),"EGFR_cnv","Patient_ID")]
  if(combos[j,1] != "Total"){
    data=data[data[,1]==as.character(combos[j,1]),] 
  }
  
  data=data[,-1]
  colnames(data)=c("Measurement","EGFR","Patient")
  ...
  model<-lmer(Measurement~EGFR+(1|Patient),data=data)
  ...
  em<-summary(emmeans(model,pairwise~EGFR,adjust="tukey"))
  if(dim(aggmeans)[1]==0){
  ...
  if (length(sig_genotypes)>0){
    ...
    plot=ggplot(meanse,aes(x=as.character(unlist(meanse$EGFR)),y=emmean,fill=as.character(unlist(meanse$EGFR))))+geom_bar(stat="identity")+geom_errorbar(aes(ymin=emmean-SE,ymax=emmean+SE),size=1)+scale_fill_manual(values=c("royalblue3","steelblue","grey"))+theme_bw()+
        xlab("")+ylab("Value")+ggtitle(title)+theme(legend.position = "none")
    png(paste(combos[j,1],gsub(".Raw_Mean","",combos[j,2]),"specifics.png",sep="_"))
    print(plot)
    dev.off()
...
colnames(aggpval)=c("MRI","Measurment","EGFR")
aggres[,2]=gsub(".Raw_Mean","",aggres[,2])
aggres=aggres[order(aggres[,1]),]
write.csv(aggres,"EGFR_percentvariance_attributed.csv")
write.csv(aggpval,"EGFR_pvalue.csv")
