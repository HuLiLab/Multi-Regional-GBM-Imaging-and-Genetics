...
rawdata=cbind.data.frame(rawdata)
...
rawdata$CDKN2A_cnv[rawdata$CDKN2A_cnv==0 ]="WT"
rawdata$CDKN2A_cnv[rawdata$CDKN2A_cnv==-1 ]="Loss-1"
rawdata$CDKN2A_cnv[rawdata$CDKN2A_cnv==-2 ]="Loss-2"
rawdata$CDKN2A_cnv=factor(rawdata$CDKN2A_cnv,levels=c("WT","Loss-1","Loss-2"))

#keep samples with genomics and imaging data
rawdata=rawdata[rowSums(rawdata[,5:9], na.rm = TRUE)>0,]
rawdata=rawdata[!is.na(rawdata$EGFR_cnv),]
rawdata=rawdata[!is.na(rawdata$CDKN2A_cnv),]
...
for(j in 1:length(combos[,1])){
  data=rawdata[,c("MRI.contrast.enhancing.annotation",as.character(combos[j,2]),"EGFR_cnv","CDKN2A_cnv","Patient_ID")]
  if(combos[j,1] != "Total"){
    data=data[data[,1]==as.character(combos[j,1]),] 
  }
  data=data[,-1]
  colnames(data)=c("Measurement","EGFR","CDKN2A","Patient")
  ...
  model<-lmer(Measurement~EGFR*CDKN2A+(1|Patient),data=data)
  ...
  em<-summary(emmeans(model,pairwise~EGFR:CDKN2A,adjust="tukey"))
  if(dim(aggmeans)[1]==0){
  ...
  em.cdk=summary(contrast(emmeans(model,"CDKN2A"),"tukey"))
  ...
  if(dim(aggcdk)[1]==0){
    em.cdk[,2]=paste(combos[j,1],combos[j,2],sep="_")
    aggcdk=em.cdk[,c(1,2,6)]
  }
  else {
    em.cdk[,2]=paste(combos[j,1],combos[j,2],sep="_")
    aggcdk=rbind.data.frame(aggcdk,em.cdk[,c(1,2,6)])
  }
  write.csv(aggcdk,paste(combos[j,1],combos[j,2],"cdk_contrasts.csv",sep="_"))
  ...
  if (length(sig_genotypes)>0){
    ...
    for(i in 1:length(sig_genotypes[,1])){
      ...
      model<-lmer(Measurement~Group+EGFR+CDKN2A+(1|Patient),data=curr)
      ...
      plot=ggplot(em.group.pval,aes(x=Group,y=emmean,fill=Group))+geom_bar(stat="identity")+geom_errorbar(aes(ymin=emmean-SE,ymax=emmean+SE),size=1)+scale_fill_manual(values=c("steelblue1","grey"))+theme_classic()+
...
colnames(aggpval)=c("MRI","Measurment","EGFR","CDKNN2A","EGFR-CDKN2A")
aggres[,2]=gsub(".Raw_Mean","",aggres[,2])
aggres=aggres[order(aggres[,1]),]

write.csv(aggres,"EGFR_CDKN2A_percentvariance_attributed.csv")
write.csv(aggpval,"EGFR_CDKN2A_pvalue.csv")
